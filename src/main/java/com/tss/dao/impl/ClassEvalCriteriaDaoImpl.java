...
public class ClassEvalCriteriaDaoImpl implements ClassEvalCriteriaDao{
    ...
    @Override
    public List<ClassEvalCriteria> findAll(Connection connection, int start, int length, String search, String mId , String assId , String status) throws SQLException {
...
"WHERE\n" +
"    criteria_name LIKE ?\n" +
"        AND m.milestone_id LIKE ? AND a.ass_id LIKE ? AND e.status_id LIKE ?\n" +
"ORDER BY criteria_id ASC\n" +
"LIMIT ? , ?";
            Object[] params = {"%" + search + "%", "%" + mId + "%", "%" + assId + "%","%" + status + "%", start, length};
            try {
