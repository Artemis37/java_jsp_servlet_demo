...
public class TraineeDaoImpl implements TraineeDao {
    ...
    @Override
    public void update(Connection connection, int userId , int classId , float grade) throws SQLException {
        PreparedStatement preparedStatement = null;
        if (connection != null) {
            String sql = "UPDATE class_user SET  grade = ? WHERE user_id = ? and class_id = ?;";
            Object[] params = { grade, userId , classId };
            try {
                BaseDao.execute(connection, preparedStatement, sql, params);
            } catch (SQLException e) {
                DebugHelper.print(e);
